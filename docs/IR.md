...
> Visible effects in the source — move, borrow, clone, drop, share, weak, raw, capture mode, failure propagation, and compile-time execution — must remain visible as Tags in the IR. Cosmetic head-renames clean up parser noise. No type info is added at this stage; M2/M3 do that.
...
(pin expr)   ; @x   parsed/reserved pin marker; no V1 semantic contract until pin design lands
