...
The semantic IR is what M2 (ownership checker) and M3 (Zig emitter) consume. It comes from the **Sexer** (sexp rewriter) — `Sexer` in `src/rig.zig`, which walks the raw output of the **Parser** (sex-stage raw producer in `src/parser.zig`, generated by Nexus) and applies the rules below.

For the architectural picture, see [CHECKLIST.md](CHECKLIST.md#architecture--lexer--sexer-with-rewriters). In short: every Nexus-based language has a *parser* with two stages, **lex** and **sex**, each with a Nexus-generated raw producer (`BaseLexer`, `Parser`) and an optional language-specific rewriter (`Lexer` for tokens, `Sexer` for sexps) that lives in `{lang}.zig`.
