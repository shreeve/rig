...
## Architecture — Lexer + Sexer with rewriters

A Nexus-based compiler has a single conceptual *parser* with two stages,
**lex** (text → tokens) and **sex** (tokens → S-expressions). Each stage
has two pieces:

```
                    GENERATOR (raw)              REWRITER (optional, lang-specific)
                    ─────────────────            ─────────────────────────────────
   lex stage:       BaseLexer                    Lexer  (token rewriter)
   sex stage:       Parser  (will rename:        Sexer  (sexp rewriter)
                    BaseSexer in Nexus)
```

| File | Origin | Contents |
|------|--------|----------|
| `src/parser.zig` | Nexus-generated, regenerated by `zig build parser` | `BaseLexer`, `Token`, `TokenCat`, `Parser`, `Sexp` |
| `src/rig.zig` | Hand-written `@lang` module | `Tag` enum, `BindingKind` + `bindingKindOf`, `KeywordId` + `keywordAs`, `Lexer` (token rewriter), `Sexer` (sexp rewriter) |

Both rewriters are optional. If `rig.zig` doesn't define `Lexer`, Nexus uses
`BaseLexer` directly (Nexus contract: `@hasDecl(rig, "Lexer")`). The Sexer
has no auto-wire today; `main.zig` instantiates it explicitly. (Auto-wiring
the Sexer is queued for the next Nexus minor-version bump — see "Nexus
suggestions" below.)

The "lex / sex" framing is the project's mental model. The names are
honest:

- `lex` is the long-standing verb for tokenization (lexeme processor).
- `sex` is the analogous verb for S-expression construction (sexp processor).

We embrace the symmetry. Casual prose may use "lexer fixup" / "sexer fixup"
for the rewriter wrappers; the canonical term is **rewriter**, and in code
they're just `Lexer` and `Sexer`.

## M0 — Parser online (Lexer (token rewriter) + raw Sexer)
...
## M1 — Sexer (sexp rewriter)

- [x] `docs/SEMANTIC-SEXP.md`
- [x] `Sexer` in `src/rig.zig` implements every SPEC §"Semantic IR Nodes" form
...
- [x] `Normalizer` (the M1 sexp rewriter) renamed to `Sexer` and consolidated into `src/rig.zig` next to `Lexer`. `src/normalize.zig` deleted. The `@lang` module (`rig.zig`) now contains both fixups + `Tag` + `BindingKind` + `KeywordId`. Architecture documented in this file under "Architecture — Lexer + Sexer with fixups."
...
- **Rename Nexus's generated `Parser` to `BaseSexer`.** Today the lex stage exposes `BaseLexer` (clearly the raw producer) but the sex stage exposes `Parser` (which is also a raw producer — but the name doesn't reflect that). Renaming gives perfect symmetry: `BaseLexer` + `Lexer` (fixup) for the lex stage; `BaseSexer` + `Sexer` (fixup) for the sex stage. The user-visible `bin/rig parse <file>` subcommand can keep its colloquial name; only the generated Zig type changes. Optional Nexus enhancement: auto-wire `Sexer` from the `@lang` module the same way it auto-wires `Lexer` (currently we instantiate `Sexer` ourselves from `main.zig`).
