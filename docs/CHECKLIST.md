...
## Architecture — Lexer + Sexer with fixups
...
```
                    GENERATOR (raw)              LANGUAGE FIXUP (optional)
                    ─────────────────            ──────────────────────────
   token stage:     BaseLexer                    Lexer
   sexp stage:      Parser  (will rename:        Sexer
                    BaseSexer in Nexus)
```
...
| `src/rig.zig` | Hand-written `@lang` module | `Tag` enum, `BindingKind` + `bindingKindOf`, `KeywordId` + `keywordAs`, `Lexer` (lex fixup), `Sexer` (sex fixup) |

Both fixups are optional. If `rig.zig` doesn't define `Lexer`, Nexus uses
`BaseLexer` directly (Nexus contract: `@hasDecl(rig, "Lexer")`). The Sexer
has no auto-wire today; `main.zig` instantiates it explicitly.
...
for the rewriter wrappers; in code they're `Lexer` and `Sexer`.
