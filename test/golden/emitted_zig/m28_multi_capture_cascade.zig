...
pub const IntSource = struct {
    ...
    pub fn set(self: IntSource, v: i32) void {
        ...
        var __rig_alive_current: bool = true;
        defer if (__rig_alive_current) { __rig_alive_current = false; current.__rig_drop(); }; _ = &current;
        if (current.buf) |__rig_p_1307| {
            var __rig_i_1307: usize = 0;
            while (__rig_i_1307 < current.len) : (__rig_i_1307 += 1) {
                const __rig_elem_1307 = &__rig_p_1307[__rig_i_1307];
                {
                    __rig_elem_1307.*.value.invoke();
                }
            }
        }
...
pub fn main() void {
    ...
    const total = IntSource.new(0);
    var __rig_alive_total: bool = true;
    defer if (__rig_alive_total) { __rig_alive_total = false; total.dropStrong(); };
    const body_a: *rig.RcBox(rig.Closure0) = rig_closure_0: {
        const Env = struct {
            cap_count: *rig.RcBox(IntSource),
            cap_total: *rig.RcBox(IntSource),
            fn rigInvoke(ctx: *anyopaque) void {
                const self: *@This() = @ptrCast(@alignCast(ctx));
                self.cap_total.value.set((self.cap_count.value.get() * 10));
            }
            fn rigDrop(ctx: *anyopaque, allocator: std.mem.Allocator) void {
                const self: *@This() = @ptrCast(@alignCast(ctx));
                self.cap_count.dropStrong();
                self.cap_total.dropStrong();
                allocator.destroy(self);
            }
        };
        const __rig_env = rig.defaultAllocator().create(Env) catch @panic("Rig closure env allocation failed");
        __rig_env.* = .{ .cap_count = count.cloneStrong(), .cap_total = total.cloneStrong() };
    ...
    var __rig_alive_body_a: bool = true;
    defer if (__rig_alive_body_a) { __rig_alive_body_a = false; body_a.dropStrong(); };
    count.value.subscribe(body_a.cloneStrong());
    const body_b: *rig.RcBox(rig.Closure0) = rig_closure_1: {
        const Env = struct {
            cap_total: *rig.RcBox(IntSource),
            fn rigInvoke(ctx: *anyopaque) void {
                const self: *@This() = @ptrCast(@alignCast(ctx));
                std.debug.print("{any}\n", .{ self.cap_total.value.get() });
            }
            fn rigDrop(ctx: *anyopaque, allocator: std.mem.Allocator) void {
                const self: *@This() = @ptrCast(@alignCast(ctx));
                self.cap_total.dropStrong();
        ...
        const __rig_env = rig.defaultAllocator().create(Env) catch @panic("Rig closure env allocation failed");
        __rig_env.* = .{ .cap_total = total.cloneStrong() };
    ...
    var __rig_alive_body_b: bool = true;
    defer if (__rig_alive_body_b) { __rig_alive_body_b = false; body_b.dropStrong(); };
    total.value.subscribe(body_b.cloneStrong());
    count.value.set(1);
    count.value.set(7);
    std.debug.print("{any}\n", .{ total.value.get() });
}
