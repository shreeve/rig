...
pub fn main() void {
    ...
    const cb3: *rig.RcBox(rig.Closure0) = rig_closure_2: {
        const Env = struct {
            cap_count: *rig.RcBox(rig.Cell(i32)),
            fn rigInvoke(ctx: *anyopaque) void {
                const self: *@This() = @ptrCast(@alignCast(ctx));
                self.cap_count.value.set((self.cap_count.value.get() + 100));
            }
            fn rigDrop(ctx: *anyopaque, allocator: std.mem.Allocator) void {
                const self: *@This() = @ptrCast(@alignCast(ctx));
                self.cap_count.dropStrong();
    ...
    var __rig_alive_cb3: bool = true;
    defer if (__rig_alive_cb3) { __rig_alive_cb3 = false; cb3.dropStrong(); };
