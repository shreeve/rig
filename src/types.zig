...
fn propagateDropGlue(ctx: *SemContext) void {
    ...
    while (changed) {
        ...
        for (ctx.symbols.items) |*sym| {
            ...
            for (fields) |f| {
                const glue = f.is_drop_method or
                    (!f.is_method and !f.is_variant and typeHasDropGlue(ctx, f.ty));
                if (glue) {
...
pub fn typeHasDropGlue(ctx: *const SemContext, ty_id: TypeId) bool {
    ...
    return switch (ctx.types.get(ty_id)) {
        ...
        .parameterized_nominal => |pn| blk: {
            ...
            if (pn.sym == ctx.cell_sym_id) break :blk pn.args.len == 1 and typeHasDropGlue(ctx, pn.args[0]);
            break :blk ctx.symbols.items[pn.sym].flags.has_drop_glue;
        },
        .nominal => |s| ctx.symbols.items[s].flags.has_drop_glue,
        else => false,
    };
}
