...
pub fn notEquatable(ctx: *SemContext, ty: TypeId, params: ?*std.ArrayListUnmanaged(SymbolId)) std.mem.Allocator.Error!?NotEquatable {
    ...
    return walk.check(ctx, ty, false);
}
...
const EquatableWalk = struct {
    ...
    /// `ty` in the type store of `at`; `in_decl` when a field or payload
    /// holds it, where a slice is a borrow.
    fn check(self: *EquatableWalk, at: *const SemContext, ty: TypeId, in_decl: bool) std.mem.Allocator.Error!?NotEquatable {
        const fail: NotEquatable = .{ .ctx = at, .ty = ty, .path = "", .why = .handle };
        switch (at.types.get(ty)) {
            .optional => |inner| return self.check(at, inner, in_decl),
            .array => |a| return self.check(at, a.elem, in_decl),
            .slice => |s| return if (in_decl) with(fail, .borrow) else self.check(at, s.elem, in_decl),
    ...
    fn checkDecl(self: *EquatableWalk, at: *const SemContext, ty: TypeId) std.mem.Allocator.Error!?NotEquatable {
        ...
        for (fields) |*f| {
            for (dataFields(f)) |d| {
                ...
                var inner = (try self.check(decl.ctx, fty, true)) orelse continue;
